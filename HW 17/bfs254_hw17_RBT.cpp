...
/*
Modifier: Briana Singleton
NYU ID: bfs254
HW 17 
Added single clockwise rotation funtionality to singleCR function, single counterclockwise rotation functionality to singleCCR function,
 and red black tree re-balancing rules to insertion function
...
template <class T>
void RBT<T>::doubleCCR(RBTNode<T> *&point) { //right-left rotation
    singleCR(point->right);
    singleCCR(point);
}

template <class T>
void RBT<T>::doubleCR(RBTNode<T> *&point) { //left-right rotation
    singleCCR(point->left);
    singleCR(point);
}

template <class T>
void RBT<T>::singleCR(RBTNode<T> *&point) { // inserted point of problem section passed in

    RBTNode<T> *grandparent = point->parent->parent; 
    RBTNode<T>* greatGrandparent = point->parent->parent->parent;
    RBTNode<T> *parent = point->parent; 

    // TODO: ADD ROTATION CODE HERE
    if (grandparent->parent == nullptr) { //if grandparent is root, make parent new root
        root = parent;
        ...
        if(parent->right == nullptr) {
            ...
            RBTNode<T>* parentRightKid = parent->right;
            parent->right = grandparent;
            grandparent->parent = parent;
            grandparent->right = parentRightKid;
        }
        swapColor(parent);
        swapColor(grandparent);
    }
    else {
        //change grandparent's parent's left ptr to point at parent ptr
        greatGrandparent->left = parent;
        parent->parent = greatGrandparent;
        //parent's right pointer points at grandparent, grandparent's parentPtr equals parent
        parent->right = grandparent;
        grandparent->parent = parent;
        grandparent->left = nullptr;
...
void RBT<T>::singleCCR(RBTNode<T> *&point) { 
    ...
    if (grandparent->parent == nullptr) {
        ...
        if (parent->left == nullptr) {
            ...
            RBTNode<T>* parentLeftKid = parent->left;
            parent->left = grandparent;
            grandparent->parent = parent;
            grandparent->right = parentLeftKid;
        }

        swapColor(parent); //turns black
        swapColor(grandparent); //turns red
    }
    else {
...
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) {
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        if (point->parent == nullptr) { // if POINT is root, swap color
            root = point;
            swapColor(point);
        }
        ...
        if (point->parent != nullptr && getColor(parent) == RED) { // if parent's color is RED
            //POINT has new node's info, PARENT is POINT's parent, GRANDPARENT is POINT's PARENT's parent

            RBTNode<T>* grandparent = parent->parent;
            RBTNode<T>* rightAunt = grandparent->right;
            RBTNode<T>* leftAunt = grandparent->left;
         

            if (parent == grandparent->left && getColor(rightAunt) == RED) { //if rightAunt is RED, swapColor()
                swapColor(parent); // turns BLACK
                swapColor(rightAunt); // turns BLACK
                swapColor(grandparent); // turns RED

                if (getColor(grandparent) == RED && getColor(grandparent->parent) == RED) {
                    if(getColor(grandparent->parent->parent->right) == BLACK){
                        singleCR(grandparent);
                    }
                    else {
                        if (grandparent->parent->parent == root) {
                            swapColor(grandparent->parent->parent->left);
                            swapColor(grandparent->parent->parent->right);
                        }
                        else {
                            swapColor(grandparent->parent->parent);
                            swapColor(grandparent->parent->parent->left);
                            swapColor(grandparent->parent->parent->right);
                        }
                    }
            ...
            else if (parent == grandparent->right && getColor(leftAunt) == RED) { //if leftAunt is RED, swapColor()
                swapColor(parent);  // turns BLACK
                swapColor(leftAunt); // turns BLACK
                swapColor(grandparent); // turns RED

                if (getColor(grandparent) == RED && getColor(grandparent->parent) == RED) {
                    if (getColor(grandparent->parent->parent->left) == BLACK){
            ...
            else if (parent == grandparent->left && getColor(rightAunt) == BLACK){ //if rightAunt is BLACK 
                if (point == parent->left) { // left left
                    singleCR(point);
                }
                else {
                    doubleCR(point); // left right
                }
                
            }
            else if (parent == grandparent->right && getColor(leftAunt) == BLACK) { //if leftAunt is BLACK
                    if (point == parent->right) { // right right
                        singleCCR(point);
                    }
                    else {
                        doubleCCR(point); // right left
                    }
            }
        }
